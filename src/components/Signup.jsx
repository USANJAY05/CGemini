...
const Signup = ({email,setEmail,password,setPassword,error,setError,navigate}) => {
    ...
    useEffect(()=>{
        try {
            ...
            if(!userCredentials) throw Error("Error")
            ...
            <div className='w-96 p-8 border border-slate-800 rounded flex flex-col gap-10'>
                ...
                <form className='w-full flex flex-col gap-6' onSubmit={handleSubmit}>
                    <div className='flex flex-col gap-1'>
                        ...
                        <input className={`px-2 p-1 rounded border border-slate-800 outline-none dark:bg-slate-950 ${error && 'border-red-500'}`} placeholder='abc@example.com' required id='email' onChange={(e)=>setEmail(e.target.value)} />
                    </div>
                    <div className='flex flex-col gap-1'>
                        <label htmlFor="password" className={error&&'text-red-500'}>Password</label>
                        <input className={`px-2 p-1 rounded border border-slate-800 outline-none dark:bg-slate-950 ${error && 'border-red-500'}`} placeholder='password' required id='password' onChange={(e)=>setPassword(e.target.value)} />
                        <small className='text-red-500'>{error}</small>
                    </div>
                    
                    <input className='p-1 bg-blue-500 rounded hover:bg-blue-600' type="submit" value="Signup" />
                </form>
                <div className='flex flex-col gap-5'>
                    ...
                    <small className='text-center'>Already have an account?<Link className='text-blue-400 font-medium' to={'/login'}> login</Link></small>
                </div>
            </div>
        </section>
